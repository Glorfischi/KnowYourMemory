...
\comment{I think we need some kind of references for that}
This results in a data race when we issue a read directly after a write. To still be able to post these two operations 
at the same time, we need to use a \code{IBV\_SEND\_FENCE} flag to the write request. When we add fence indicator to a write
