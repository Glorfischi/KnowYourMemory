...
\paragraph{} Now we will evaluate the protocols performance with multiple open connections. We still use two nodes.
...
For both 16 byte as well as 512 byte messages, the Write Reverse sender is a fair bit faster than the Write Immediate sender,
...
performance then Write Immediate for 16 byte messages and about the same throughput for 512 byte messages.~\comment{ I can't 
really explain this. We should be completely limited by the receiving CPU and writeOff is not more involved for the CPU}
