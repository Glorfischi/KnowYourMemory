...
higher performance when opening more than 7 concurrent connections with both 512 byte as well as 16 byte sized messages. As of
writing this, we do not have a clear explanation for it. Our current best guess is, that with more than 7 connection the 
metadata is spread over more than one cache line. This can result in less cache invalidation and in turn to lower receiver 
...
active sender. The use of device memory \mbox{(SW-DM)} allows us to saturate the link with only 9 connections. This is not a 
