...
\paragraph{} RDMA gives applications direct access to the RNIC through a usersspace library, completely circumventing the 
...
\begin{itemize}
  \item \textbf{Fetch and Add} reads a 64 bit value from the remote memory space, add the additionally provided 
    \code{compare\_add} value, and writes the original data before the addition to the provided local memory.
