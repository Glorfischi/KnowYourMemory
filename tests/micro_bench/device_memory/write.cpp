...
int server(std::string ip){
  ...
  struct ibv_mr *ram_mr = ibv_reg_mr(pd, ram, 16, IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_LOCAL_WRITE);
  if (ram_mr == nullptr) {
...
int client(std::string ip, int count){
  ...
  for (int i = 0; i<n; i++) {
    ...
    auto stat = ep->PostWrite(i, src_mr->lkey, src_mr->addr, 16, ci->ram_addr, ci->ram_key);
    if (!stat.ok()){
  ...
  for (int i = 0; i<n; i++) {
    auto start = std::chrono::high_resolution_clock::now();
    auto stat = ep->PostWrite(i, src_mr->lkey, src_mr->addr, 16, 0, ci->dm_key);
    if (!stat.ok()){
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "#### Testing Device Memory write latency N: " << count << std::endl;
